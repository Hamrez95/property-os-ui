...
function Toolbar({placeholder,action}:{placeholder:string,action?:string}) {
  return <div className="adminx-toolbar"><div className="global-search"><Search size={15}/><span>{placeholder}</span></div>{action&&<button className="btn primary compact"><Plus size={14}/>{action}</button>}</div>
}

function SimpleTable({columns,rows}:{columns:string[],rows:Row[]}) {
  return <div className="adminx-table">
    <div className="adminx-row header">{columns.map(c=><span key={c}>{c}</span>)}<span/></div>
    {rows.map((r,i)=><div className="adminx-row" key={r[0]+i}><span><b>{r[0]}</b></span><span>{r[1]}</span><span>{r[2]}</span><span><Status tone={r[4]}>{r[3]}</Status></span><span><MoreVertical size={15}/></span></div>)}
  </div>
}
...
export function AdminBuildings() {
  ...
  return <AdminShell section="ساختمان‌ها"><div className="admin-page-head"><div><h2>ساختمان‌ها</h2><p>ساختار، واحدها، مدیریت و عملیات مشترک</p></div></div><Toolbar placeholder="جستجو در ساختمان‌ها..." action="ساختمان جدید"/><SimpleTable columns={['عنوان','تعداد واحد','موقعیت','وضعیت']} rows={rows}/></AdminShell>
}
...
export function AdminListings() {
  ...
  return <AdminShell section="قراردادها"><div className="admin-page-head"><div><h2>آگهی‌ها</h2><p>Listing projectionها و وضعیت انتشار</p></div></div><Toolbar placeholder="جستجو در آگهی‌ها..."/><SimpleTable columns={['آگهی','نوع','بروزرسانی','وضعیت']} rows={rows}/></AdminShell>
}
...
export function AdminDeals() {
  ...
  return <AdminShell section="قراردادها"><div className="admin-page-head"><div><h2>معاملات</h2><p>Offer → Contract → Payment → Handover</p></div></div><Toolbar placeholder="شناسه یا ملک..."/><SimpleTable columns={['شناسه','ملک','ارزش','مرحله']} rows={rows}/></AdminShell>
}
...
export function AdminTrustQueue() {
  ...
  return <AdminShell section="بازرسی‌ها"><div className="admin-page-head"><div><h2>Trust & Verification</h2><p>Claimها، provenance و صف بررسی</p></div></div><div className="adminx-trust-summary"><Stat icon={ShieldCheck} label="صف بررسی" value="148" delta="-9%"/><Stat icon={ClipboardCheck} label="QA امروز" value="42" delta="+16%"/><Stat icon={CheckCircle2} label="تأییدشده" value="1,204" delta="+6%"/></div><Toolbar placeholder="Property یا Claim..."/><SimpleTable columns={['Property','Claim','منبع','وضعیت']} rows={rows}/></AdminShell>
}
...
export function AdminInspectors() {
  ...
  return <AdminShell section="بازرسی‌ها"><div className="admin-page-head"><div><h2>بازرسان</h2><p>تخصیص، کیفیت و عملکرد میدانی</p></div></div><Toolbar placeholder="جستجو در بازرسان..." action="دعوت بازرس"/><SimpleTable columns={['نام','امتیاز','این ماه','وضعیت']} rows={rows}/></AdminShell>
}
...
export function AdminPayments() {
  ...
  return <AdminShell section="پرداخت‌ها"><div className="admin-page-head"><div><h2>پرداخت‌ها</h2><p>Payment intent، وضعیت rail و reconciliation</p></div></div><Toolbar placeholder="شناسه پرداخت..."/><SimpleTable columns={['شناسه','مرجع','مبلغ','وضعیت']} rows={rows}/></AdminShell>
}
...
export function AdminSupport() {
  ...
  return <AdminShell section="پیام‌ها"><div className="admin-page-head"><div><h2>پشتیبانی</h2><p>درخواست‌ها، SLA و ارجاع داخلی</p></div></div><Toolbar placeholder="شماره تیکت یا کاربر..."/><SimpleTable columns={['تیکت','موضوع','سن','وضعیت']} rows={rows}/></AdminShell>
}
...
export function AdminFeatureFlags() {
  ...
  return <AdminShell section="تنظیمات"><div className="admin-page-head"><div><h2>Feature Flags</h2><p>Capability gates برای قابلیت‌های حساس و مجوزمحور</p></div></div><div className="flag-list">{flags.map(f=><div key={f[0]}><Flag size={16}/><span><strong>{f[0]}</strong><small>{f[1]}</small></span><i className={f[2]}><em/></i></div>)}</div></AdminShell>
}
...
export function AdminSecurityAudit() {
  ...
  return <AdminShell section="تنظیمات"><div className="admin-page-head"><div><h2>Security & Audit</h2><p>رویدادهای حساس، دسترسی و تغییرات غیرقابل انکار</p></div></div><Toolbar placeholder="شناسه یا actor..."/><SimpleTable columns={['Event','عملیات','Actor / Target','وضعیت']} rows={rows}/></AdminShell>
}
