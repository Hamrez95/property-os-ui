  ...
  return <Phone active="none">
    <section className="onboarding" aria-label={`معرفی محصول، مرحله ${step + 1} از ${slides.length}`}>
      ...
      <div className="onboarding-footer">
        ...
        <button className="btn primary onboarding-next" onClick={() => step === slides.length - 1 ? finish() : setStep(step + 1)}>{step === slides.length - 1 ? 'ورود به خانه' : <>بعدی <ArrowLeft size={16}/></>}</button>
